...
The set \(S\) is said to be \defn{bounded} if \(D(S)\ne\infty\) (equivalently,
...
(closed) subset of \(X\)'' or ``\(S\) is an open (closed) set \emph{in} \(X\)''
...
the \defn{derived set} of \(S\) in \(X\), and is denoted by \(S'\).
...
called the \defn{boundary} of \(S\), and is denoted by \(\partial S\).

\begin{remark}
\item We can see that the condition here implies the one for adherent point (we also
require that \(B(x,r)\cap (X\setminus S)\ne\varnothing\) for any \(r>0\).
Hence, we have \(\partial S\subseteq \overline{S}\).
